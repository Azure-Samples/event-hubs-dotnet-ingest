...
namespace SendSampleData
{
    class Program
    {
        ...
        static void EventHubIngestion()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                try
                {
                    ...
                    // eventData.Properties.Add("Compression", "None"); // default
        ...
        // Ingest GZip compressed data via dynamic routing
        static void GZipEventHubIngestion()
        {
            var eventHubClient = EventHubClient.CreateFromConnectionString(connectionString, eventHubName);
            int counter = 0;
            for (int i = 0; i < 100; i++)
            {
                ...
                try
                {
                    ...
                    EventData eventData = new EventData(GZipCompressString(recordString));
                    Console.WriteLine($"sending message {counter}");
        ...
        private static byte[] GZipCompressString(string stringToCompress)
        {
            byte[] compressed;
            ...
            using (var result = new MemoryStream())
            {
                ...
                {
                    compressionStream.Write(Encoding.UTF8.GetBytes(stringToCompress), 0, stringToCompress.Length);
                }
